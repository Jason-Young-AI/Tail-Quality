# Modified On "mxnet.gluon.data.vision.ImageFolderDataset"
import os
    ...
    def _list_images(self, root):
        ...
        for folder in sorted(os.listdir(root)):
            ...
            if not os.path.isdir(path):
                print('Ignoring %s, which is not a directory.'%path, stacklevel=3)
                continue
            label_id = len(self.synsets)
            self.synsets.append(folder)
            for filename in sorted(os.listdir(path)):
                filename = os.path.join(path, filename)
                ext = os.path.splitext(filename)[1]
                if ext.lower() not in self._exts:
                    print('Ignoring %s of type %s. Only support %s'%(
                        filename, ext, ', '.join(self._exts)))
                    continue
                self.items.append((filename, label_id+1))

    def __getitem__(self, idx):
        img_path = self.items[idx][0]
        label_id = self.items[idx][1]
