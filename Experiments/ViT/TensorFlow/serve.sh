...
DOCKER_CON_NAME="Serving_ViT_TF"
...
if [[ $ACTION == "start" ]]; then
	...
	#CUDA_VISIBLE_DEVICES=0 docker run --gpus '"device=0"' --name ${DOCKER_CON_NAME} -p 8500:8500 -p 8501:8501 \
	#docker run --runtime=nvidia -e NVIDIA_VISIBLE_DEVICES=0 --gpus '"device=0"' --name ${DOCKER_CON_NAME} -p 8500:8500 -p 8501:8501 \
	docker run --runtime=nvidia --gpus '"device=0"' --name ${DOCKER_CON_NAME} -p 8500:8500 -p 8501:8501 \
		--mount type=bind,source="${THIS_PATH}/bins/jx_vit_large_p32.tf",target=/models/ViT_L_P32_384 \
		--mount type=bind,source="${THIS_PATH}/configs",target=/configs \
		-v ${THIS_PATH}/tensorboard:/tmp/tensorboard \
		-e MODEL_NAME=ViT_L_P32_384 -t tensorflow/serving:2.11.0-gpu \
