...
class Transformer(tf.keras.Model):
    def __init__(self, model_dim=256, num_heads=8, num_encoder_layers=6,
        ...
        enc_norm = None
        if normalize_before:
            enc_norm = LayerNormalization(epsilon=1e-5, name='norm_pre')
        dec_norm = LayerNormalization(epsilon=1e-5, name='norm')

        self.encoder = TransformerEncoder(
            model_dim, num_heads, dim_feedforward, dropout, activation,
            normalize_before, enc_norm, num_encoder_layers, name='encoder')

        self.decoder = TransformerDecoder(
            model_dim, num_heads, dim_feedforward, dropout, activation,
            normalize_before, dec_norm, num_decoder_layers, name='decoder',
    ...
    def call(self, source, mask, query_encoding, pos_encoding, training=False):
        ...
        pos_encoding = tf.reshape(pos_encoding,
...
class TransformerEncoder(tf.keras.Model):
    def __init__(self, model_dim=256, num_heads=8, dim_feedforward=2048,
                 ...
                 norm=None, num_encoder_layers=6, **kwargs):
        super().__init__(**kwargs)
...
class TransformerDecoder(tf.keras.Model):
    def __init__(self, model_dim=256, num_heads=8, dim_feedforward=2048,
                 dropout=0.1, activation='relu', normalize_before=False,
                 norm=None, num_decoder_layers=6, return_intermediate=False,
                 **kwargs):
        super().__init__(**kwargs)
...
class EncoderLayer(tf.keras.layers.Layer):
    ...
    def call(self, source, source_mask=None, source_key_padding_mask=None,
        ...
        if self.normalize_before:
            return self.pre_norm_call(source, source_mask,
                                      source_key_padding_mask,
                                      pos_encoding, training)
        return self.post_norm_call(source, source_mask,
                                   source_key_padding_mask,
                                   pos_encoding, training)

    def pre_norm_call(self, source, source_mask=None,
                      source_key_padding_mask=None,
                      pos_encoding=None, training=False):
        raise Exception('pre_norm_call not implemented yet')

    def post_norm_call(self, source, source_mask=None,
                       source_key_padding_mask=None,
                       pos_encoding=None, training=False):
...
class DecoderLayer(tf.keras.layers.Layer):
    ...
    def pre_norm_call(self, target, memory, target_mask=None,
                      memory_mask=None, target_key_padding_mask=None,
                      memory_key_padding_mask=None, pos_encoding=None,
                      query_encoding=None, training=False):
        raise Exception('pre_norm_call not implemented yet')

    def post_norm_call(self, target, memory, target_mask=None,
                       memory_mask=None, target_key_padding_mask=None,
                       memory_key_padding_mask=None, pos_encoding=None,
                       query_encoding=None, training=False):
        ...
        attn_target2 = self.multihead_attn(
            (query_tgt, key_mem, memory), attn_mask=memory_mask,
...
class MultiHeadAttention(tf.keras.layers.Layer):
    ...
    def build(self, input_shapes):
        ...
        self.in_proj_weight = tf.Variable(
            tf.zeros((in_dim, self.model_dim), dtype=tf.float32),
            name='in_proj_kernel')
        ...
        self.out_proj_bias = tf.Variable(
            tf.zeros((self.model_dim,), dtype=tf.float32),
            name='out_proj_bias')
    ...
    def call(self, inputs, attn_mask=None, key_padding_mask=None,
        ...
        WQ = tf.reshape(
            WQ, [target_len, batch_size * self.num_heads, self.head_dim])
        WQ = tf.transpose(WQ, [1, 0, 2])

        WK = tf.reshape(
            WK, [source_len, batch_size * self.num_heads, self.head_dim])
        WK = tf.transpose(WK, [1, 0, 2])

        WV = tf.reshape(
            WV, [source_len, batch_size * self.num_heads, self.head_dim])
        ...
        if key_padding_mask is not None:
            ...
            key_padding_mask = tf.tile(
                key_padding_mask, [1, self.num_heads, target_len, 1])

            attn_output_weights = tf.where(
                key_padding_mask,
        ...
        attn_output = tf.reshape(attn_output,
                                 [target_len, batch_size, self.model_dim])
