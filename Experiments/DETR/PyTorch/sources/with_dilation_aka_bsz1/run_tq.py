...
def inference(parameters):
    ...
    if fake_run:
        ...
        with open(results_basepath.joinpath('DETR_Pytorch_origin_quality.json'), 'w') as f:
...
def draw_rjsds(rjsds: List, results_basepath: pathlib.Path):
    ...
    ax.grid(True)
    ax.legend()
    plt.savefig(results_basepath.joinpath("DETR_Pytorch_rjsds.jpg"), format="jpg")
    plt.savefig(results_basepath.joinpath("DETR_Pytorch_rjsds.pdf"), format="pdf")
...
if __name__ == "__main__":
    ...
    result_path = results_basepath.joinpath('DETR_PyTorch.pickle')
    rjsds_path = results_basepath.joinpath('DETR_PyTorch_rjsds.pickle')
